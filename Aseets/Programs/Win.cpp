...
ClWin::ClWin() {
	...
	m_size = XY(GAME_WIDTH, GAME_HEIGHT);
	return; 
}
...
//左上端を取得
XY ClWin::GetWinPivot() { return m_size.GetPivot(); }

//指定した枢軸の座標を取得
XY ClWin::GetWinPivot(n_xy::XY::PIVOT pivot) { return m_size.GetPivot(pivot); }

//枢軸Xサイズ取得
int ClWin::GetWinPivotX(n_xy::XY::ONE_PIVOT pivot_x) { return m_size.GetPivotX(pivot_x); }

//枢軸Yサイズ取得
int ClWin::GetWinPivotY(n_xy::XY::ONE_PIVOT pivot_y) { return m_size.GetPivotY(pivot_y); }
...
//画面のサイズを取得
XY ClWin::GetWinSize() { return m_size; }
...
//背景色の種類を参照
ClWin::BACK_COLOR_TYPE ClWin::GetBackColorType() { return M_BACK_COLOR_TYPE; }
