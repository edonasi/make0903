...
//フェードの種類
ClFade::FADE_TYPE ClFade::GetFadeType() { return m_fade_type; }	

//フェードを行っていないか
BOOL ClFade::GetIsFadeNone() { return (m_fade_state == n_fade::ClFade::FADE_STATE::NONE) ? TRUE : FALSE; }
...
VOID ClFade::FadeProc(){
	...
	switch (m_fade_state){
	case n_fade::ClFade::FADE_UP:
		//フェードダウンに移行
		if (m_next_in == TRUE) {
			m_next_in = FALSE;
			m_fade_state = n_fade::ClFade::FADE_STATE::FADE_DOWN;
			m_cnt.SetCnt();
		}
		else {	//終了処理
			m_cnt.SetIsStop();
			m_fade_state = n_fade::ClFade::FADE_STATE::NONE;
			m_is_start = FALSE;
		}
		break;
	...
	case n_fade::ClFade::FADE_DOWN:
		...
		if (m_next_in == TRUE) {
			m_next_in = FALSE;
			m_fade_state = n_fade::ClFade::FADE_STATE::FADE_UP;
			m_cnt.SetCnt();
		}
		else {	//終了処理
			m_cnt.SetIsStop();
