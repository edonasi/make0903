...
//画像をメモリから消去
VOID ClImg::Delete() { DeleteGraph(m_handle); }
...
VOID ClImg::Move(MOVE_LINE_TYPE move_type) {
	...
	switch (move_type) {
		//STOPのときは処理を終了
	case n_img::ClImg::STOP:return; break;

	case n_img::ClImg::UP_LEFT:
	case n_img::ClImg::LEFT:
	case n_img::ClImg::DOWN_LEFT:
		m_pos.x += -ClFps::GetIns()->GetFpsIntFix(m_abs_speed, &m_over_speed);
		break;

	case n_img::ClImg::UP_RIGHT:
	case n_img::ClImg::RIGHT:
	case n_img::ClImg::DOWN_RIGHT:
		m_pos.x += ClFps::GetIns()->GetFpsIntFix(m_abs_speed, &m_over_speed);
		break;
...
VOID ClImg::Move(int x_mult, int y_mult, n_img::ClImg::MOVE_MULT move_mult) {
	switch (move_mult)
	{
	case n_img::ClImg::ABS_SPEED:
		m_pos.x += x_mult * m_abs_speed, & m_over_speed;
		m_pos.y += y_mult * m_abs_speed, & m_over_speed;
		break;
	case n_img::ClImg::DELTA:
		m_pos.x += ClFps::GetIns()->GetFpsIntFix(x_mult * m_abs_speed, &m_over_speed);
		m_pos.y += ClFps::GetIns()->GetFpsIntFix(y_mult * m_abs_speed, &m_over_speed);
		break;
	default:
...
VOID ClImg::SetPos(XY::PIVOT win_pivot, XY::PIVOT img_pivot) {
...
VOID ClImg::SetPos(
	XY::PIVOT win_pivot, XY::PIVOT img_pivot,
...
VOID ClImg::SetPos(ClImg base_img,
	XY::PIVOT base_pivot, XY::PIVOT this_pivot) {
