...
//XYクラス構造体
n_mouse::ClMouse::XY::XY() {
    n_mouse::ClMouse::XY::x = 0;
    n_mouse::ClMouse::XY::y = 0;
}
n_mouse::ClMouse::XY::~XY() { return; }
...
ClMouse::MOUSE::MOUSE() {
    //マウス構造体の初期化
    ClMouse::MOUSE::input_value = 0;
    ClMouse::MOUSE::wheel_value = 0;
    ClMouse::MOUSE::point = n_mouse::ClMouse::XY();
    //マウスカウンタの初期化
    for (int i = 0; i < MOUSE_BUTTON_CODE; i++) { ClMouse::MOUSE::button[i] = UPPING_NUM; }
}

ClMouse::MOUSE::~MOUSE() { return; }
...
int ClMouse::MouseUpdate() {
    //マウスの座標を取得
    GetMousePoint(&m_mouse.point.x, &m_mouse.point.y);  
    
    //プレイ画面上のマウスの座標を画面内に制限する
    if (IS_MOUSE_POS_LIMIT == TRUE) {
        if (m_mouse.point.x < LIMIT_SCREEN.left) { m_mouse.point.x = LIMIT_SCREEN.left; }
        if (m_mouse.point.x > LIMIT_SCREEN.right) { m_mouse.point.x = LIMIT_SCREEN.right; }
        if (m_mouse.point.y < LIMIT_SCREEN.top) { m_mouse.point.y = LIMIT_SCREEN.top; }
        if (m_mouse.point.y > LIMIT_SCREEN.bottom) { m_mouse.point.y = LIMIT_SCREEN.bottom; }
    }

    //マウスの押しているボタンを取得
    m_mouse.input_value = GetMouseInput();

    //マウスコードに対応したボタンが押されているかチェック
    MmMouseCheck(&m_mouse, MOUSE_INPUT_LEFT);       //左
    MmMouseCheck(&m_mouse, MOUSE_INPUT_MIDDLE);     //中
    MmMouseCheck(&m_mouse, MOUSE_INPUT_RIGHT);      //右

    //ホイールの回転量を取得
    m_mouse.wheel_value = GetMouseWheelRotVol();
...
VOID ClMouse::MmMouseCheck(MOUSE* mouse, int mouse_input_) {
    //押しているとき
    if ((mouse->input_value & mouse_input_) == mouse_input_){
        //押した瞬間のときクリック開始判定を行う
        if (mouse->button[mouse_input_] == UPPING_NUM) {
            //m_is_click_start[mouse_input_] = MmIsMousePointInPlayScreen(m_mouse, mouse_input_);
        }
        mouse->button[mouse_input_]++;  //加算
    }
    //押していないとき
    else if ((mouse->input_value & mouse_input_) != mouse_input_){
        if (mouse->button[mouse_input_] > UPPING_NUM) { //前までボタンが押されていた状態だったら
            mouse->button[mouse_input_] = UP_NUM;   //-1にする
           // m_is_click_start[mouse_input_] = FALSE; //画面上どの座標にいても離した瞬間、フラグを下す
        }
        else {  
            mouse->button[mouse_input_] = UPPING_NUM;   //0にする
        }
    }
}
...
int ClMouse::GetButtonCnt(int mouse_input_) { return m_mouse.button[mouse_input_]; }
...
BOOL ClMouse::GetIsButton(int mouse_input_) {
    if (m_mouse.button[mouse_input_] > UPPING_NUM) { return TRUE; } //押されている
...
BOOL ClMouse::GetIsButtonDown(int mouse_input_) {
    if (m_mouse.button[mouse_input_] == DOWN_NUM) { return TRUE; } //押された
    else { return FALSE; }  //押されていない
}
...
BOOL ClMouse::GetIsButtonUp(int mouse_input_) {
    if (m_mouse.button[mouse_input_] == UP_NUM) { return TRUE; } //離された
...
BOOL ClMouse::GetIsButtonAway(int mouse_input_) {
    if (m_mouse.button[mouse_input_] <= UPPING_NUM) { return TRUE; } //離している
...
BOOL ClMouse::GetIsButtonTimeInput(int mouse_input_, float time) {
    ...
    int button = m_mouse.button[mouse_input_];
...
VOID ClMouse::MousePosDraw() {
    ...
    if (ClWin().GetIsDebug() == TRUE) {
        ...
        DrawFormatString(
            ...
            ClCom().GetFlipColor(), "MOUSE[X:%4d/Y:%4d]", m_mouse.point.x, m_mouse.point.y
        );
    }
    return;
}