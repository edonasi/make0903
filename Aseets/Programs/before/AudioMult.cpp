	...
	for (unsigned int i = 0; i < m_load_audio.size(); i++) {
		...
		if (m_fade.GetIsFadeStop() == FALSE) {
			...
			switch (m_fade.GetFadeType()){
				//音量割合を1.0fに
			case n_fade::ClFade::FADE_TYPE::FADE_DOWN_UP_TYPE:
			case n_fade::ClFade::FADE_TYPE::FADE_UP_TYPE:
				ChangeVolumeSoundMem(
				...
				//音量割合を0.0fに
			case n_fade::ClFade::FADE_TYPE::FADE_UP_DOWN_TYPE:
			case n_fade::ClFade::FADE_TYPE::FADE_DOWN_TYPE:
				ChangeVolumeSoundMem(
